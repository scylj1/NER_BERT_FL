'''
Analyse raw results data, calculate mean and stadard deviation
'''
import pandas as pd
import numpy as np

res = pd.read_table('val_result/res.txt', header=None, names=['model',	'seed',	'epoch', 'acc',	'pre', 'rec', 'f1']).dropna()
res_an = pd.read_table('val_result/res_an.txt', header=None, names=['model', 'epoch', 'acc', 'pre', 'rec', 'f1', 'stdacc','stdpre','stdrec','stdf1']).dropna()
...
res_an.to_csv('val_result/res_an.txt', sep='\t', index=False)
print('finished')